...
import { isNight, toKebabCase } from '../helpers';
...
export function loadTheme(
  ...
  const _theme = theme || DEFAULT_THEME_PAIR;
  ...
  if (_theme.id === 'auto' || !_theme.id) {
    ...
    const darkTheme: Theme = _preferredDarkTheme || themes[DEFAULT_DARK_THEME]!;
    const lightTheme: Theme = _preferredLightTheme || themes[DEFAULT_LIGHT_THEME]!;
  ...
  return (
    themes[_theme.id] ||
    loadTheme({ id: 'auto', color: _theme.color }, preferredDarkTheme, preferredLightTheme)
  );
}

export const defaultTheme = loadTheme(DEFAULT_THEME_PAIR);
...
themeColorFields.forEach(field => {
  cssVariablesTheme[field] = `var(--theme-${toKebabCase(field)})`;
  cssVariablesTheme[`inverted-${field}`] = `var(--theme-inverted-${toKebabCase(field)})`;
});

export function getCSSVariable(color: keyof ThemeColors, isInverted?: boolean) {
  const field =
    color && typeof color === 'string' ? (isInverted ? `inverted-${color}` : color) : undefined;

  if (field && field in cssVariablesTheme) {
    return cssVariablesTheme[field];
  }

  return color;
}

export function transformTheme(theme: Theme, themeTransformer: ThemeTransformer) {
  if (
